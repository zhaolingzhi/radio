...
flags.DEFINE_string("data_path", '/home/zlz/Radio-Prediction/simple-examples/data/', "data_path")
...
class PTBModel(object):
    ...
    def __init__(self, is_training, config):
        ...
        #outputs:shape(20*20*200)
        #outputc = tf.reshape(tf.concat(1, outputs), [-1, size])
        outputc = tf.reshape(outputs, [-1, size])

        #outputc:shape(400*200)
        with tf.variable_scope("fc_1"):
            fc_1w = tf.Variable(tf.truncated_normal([size, fc1_size],stddev=1.0 / math.sqrt(float(size))) , name='fc_1w')
            fc_1b = tf.get_variable("fc_1b", [fc1_size], dtype=data_type())
            output1=tf.nn.relu(tf.matmul(outputc,fc_1w)+fc_1b)

        # with tf.variable_scope("fc_2"):
        #     fc_2w = tf.Variable(tf.truncated_normal([fc1_size, fc2_size], stddev=1.0 / math.sqrt(float(fc1_size))),
        #                         name='fc_2w')
        #     fc_2b = tf.get_variable("fc_2b", [fc2_size], dtype=data_type())
        #     output2 = tf.nn.relu(tf.matmul(output1, fc_2w) + fc_2b)

        # 把之前的list展开，成[batch, hidden_size*num_steps],然后 reshape, 成[batch*numsteps, hidden_size]
        #output = tf.reshape(tf.concat(1, output2), [-1, size])

        # softmax_w , shape=[hidden_size, vocab_size], 用于将distributed表示的单词转化为one-hot表示
        softmax_w = tf.get_variable(
            "softmax_w", [fc1_size, vocab_size], dtype=data_type())
...
def run_epoch(session, model, data, label, eval_op, verbose=False):
...
if __name__=='__main__':
    ...
    X_train = X[train_idx]  #train data
    X_test = X[test_idx]
    ...
    Y_train = to_onehot(map(lambda x: mods.index(lbl[x][0]), train_idx))   #train label
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        for i in range(config.max_max_epoch):   # 所有文本要重复多次进入模型训练
            ...
            train_perplexity = run_epoch(session, m, X_train,Y_train, m.train_op,verbose=True) # 训练困惑度
            print("Epoch: %d Train Perplexity: %.3f" % (i + 1, train_perplexity))
            # valid_perplexity = run_epoch(session, mvalid, valid_data, tf.no_op()) # 检验困惑度
            # print("Epoch: %d Valid Perplexity: %.3f" % (i + 1, valid_perplexity))

        test_perplexity = run_epoch(session, mtest, X_test,Y_test, tf.no_op())  # 测试困惑度
